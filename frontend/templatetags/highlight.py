...
from dudalibrary.utils import normalize_query

from dudalibrary.highlighting import Highlighter

register = template.Library()

@register.filter
def highlight_only(text, words):
    if words != '':
        words = normalize_query(words)
        for word in words:
            if word:
                reg_word = word
                pattern = re.compile(re.escape(word),re.I)
                text = mark_safe(re.sub(pattern,"<span class='highlighted'>%s</span>" % word,text, 0))
            else:
                return text
    return text
...
@register.tag
def highlight_truncate(parser, token):
    """
